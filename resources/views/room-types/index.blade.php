    ...
    <div class="row mt-3">
        <div class="card">
            <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        @can('edit rooms')
                                            <a href="javascript:void(0)" data-url="{{ route('room-types.update', $item->id) }}"
                                                data-name="{{ $item->name }}"
                                        ...
                                        @can('delete rooms')
                                            <a href="javascript:void(0)"
                                                data-url="{{ route('room-types.destroy', [$item->id]) }}"
                ...
                <div class="row">
                    <h4 class="topic">Add Room Type</h4>
                    <div class="mt-3">
                        <form method="POST" class="ajax-form" action="{{ route('room-types.store') }}">
                            @csrf
                            <div class="row">
                                ...
                                {{-- <div class="col-md-6 mb-3 required">
                                    ...
                                    <select name="facility" id="facility" class="form-select" required>
                                        ...
                                        @foreach ($data1 as $data11)
                                            <option value="{{ $data11->id }}">{{ $data11->name }}</option>
                                        @endforeach
